...
function ResetPasswordForm() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (password.length < 8 || !/[A-Za-z]/.test(password) || !/\d/.test(password)) { setError("Password must be at least 8 characters and contain a letter and number."); return; }
    setLoading(true);
    try {
      ...
      <div className="bg-card border border-border rounded-xl p-8 w-full max-w-sm">
        ...
        {!token ? (
          ...
          <form onSubmit={submit}>
            ...
            <input type="password" required minLength={8} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full border border-border rounded-lg px-3 py-2 mb-2 bg-[#FCFAF5]" />
            <p className="text-xs text-inkSoft mb-4">At least 8 characters with a letter and number.</p>
            <label className="block text-xs font-semibold text-inkSoft uppercase mb-1">Confirm new password</label>
            <input type="password" required minLength={8} autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full border border-border rounded-lg px-3 py-2 mb-4 bg-[#FCFAF5]" />
