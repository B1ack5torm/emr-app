...
export default function RegisterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!organizationId) { setError("Please select an organization to join."); return; }

    setLoading(true);
    const res = await fetch("/api/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mode: "join", name, email, password, organizationId }),
    });
    ...
    setSuccessMsg("Request sent — an administrator at that organization needs to approve your account before you can sign in.");
  };
    ...
    <div className="min-h-[70vh] flex items-center justify-center">
      <form onSubmit={submit} className="bg-card border border-border rounded-xl p-8 w-full max-w-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="font-serif font-bold text-lg">Request an account</div>
        </div>
