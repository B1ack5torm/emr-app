...
type Doctor = { id: string; name: string; organizationId: string; organization: { name: string } };
type Visitor = { name: string; email: string; phone: string; reason: string };
type Confirmation = { doctor: Doctor; date: string; time: string };
...
export default function PublicAppointmentPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/public/appointments/availability?doctorId=${encodeURIComponent(doctorId)}&date=${date}`)
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/public/appointments", {
        ...
        body: JSON.stringify({ ...visitor, doctorId, organizationId: doctor.organizationId, date, time }),
      });
      const data = await response.json();
      if (!response.ok) { if (response.status === 409) setTime(""); throw new Error(data.error || "Could not send the appointment request."); }
      setComplete({ doctor, date, time });
    } catch (reason) {
      ...
      {complete ? <Success complete={complete} /> : <div className="mt-12 grid gap-8 lg:grid-cols-[.75fr_1.25fr]">
        ...
        <form onSubmit={submit} className="rounded-2xl border border-[#D7E3DD] bg-white p-5 shadow-xl shadow-[#214F43]/8 sm:p-7">
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Doctor" full><select required value={doctorId} onChange={(event) => { setDoctorId(event.target.value); setDate(""); }} className="booking-input"><option value="">Select an available doctor</option>{doctors.map((item) => <option key={item.id} value={item.id}>Dr. {item.name} — {item.organization.name}</option>)}</select></Field>
...
function Success({ complete }: { complete: Confirmation }) {
  return <div className="mx-auto mt-16 max-w-lg rounded-2xl border border-[#CFE0D8] bg-white p-8 text-center shadow-xl shadow-[#214F43]/10"><CheckCircle2 size={48} className="mx-auto text-[#24705D]" /><h1 className="mt-5 font-serif text-3xl font-bold">Request sent</h1><p className="mt-3 text-[#60776E]">Your request was sent to <b>Dr. {complete.doctor.name}</b> at <b>{complete.doctor.organization.name}</b>.</p><div className="mx-auto mt-6 max-w-sm rounded-xl bg-[#EAF4EE] p-4"><p className="font-bold">{new Date(`${complete.date}T${complete.time}:00`).toLocaleString(undefined, { dateStyle: "full", timeStyle: "short" })}</p><p className="mt-1 text-xs text-[#60776E]">This time is reserved while the doctor reviews your request.</p></div><p className="mt-5 text-sm text-[#60776E]">You will receive an email after the doctor confirms the appointment.</p><Link href="/" className="mt-7 inline-block rounded-lg bg-[#24705D] px-5 py-2.5 text-sm font-bold text-white">Return home</Link></div>;
}
