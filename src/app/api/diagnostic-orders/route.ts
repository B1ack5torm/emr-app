...
export async function GET(request: NextRequest) {
  ...
  const [orders, total] = await prisma.$transaction([prisma.diagnosticOrder.findMany({ where, include: { patient: { select: { id: true, mrn: true, name: true } }, orderingPractitioner: { select: { id: true, name: true } }, documents: { select: { id: true, originalName: true } } }, orderBy: { createdAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize }), prisma.diagnosticOrder.count({ where })]);
