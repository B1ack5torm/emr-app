...
export async function POST(request: NextRequest) {
  ...
  const schedule = await prisma.practitionerSchedule.upsert({ where: { practitionerId_dayOfWeek: { practitionerId: profile.id, dayOfWeek } }, update: { startMinute, endMinute, appointmentMinutes, active: true, breaks: { deleteMany: {}, create: breaks } }, create: { practitionerId: profile.id, dayOfWeek, startMinute, endMinute, appointmentMinutes, breaks: { create: breaks } }, include: { breaks: true } });
