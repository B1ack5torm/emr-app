...
export async function POST(request: NextRequest) {
  ...
  if (typeof token !== "string" || typeof password !== "string" || password.length < 12) return NextResponse.json({ error: "Use a valid reset token and a password of at least 12 characters." }, { status: 400 });
  ...
  await prisma.$transaction([prisma.user.update({ where: { id: user.id }, data: { passwordHash: await bcrypt.hash(password, 12), mustChangePassword: false } }), prisma.passwordResetToken.update({ where: { id: reset.id }, data: { usedAt: new Date() } })]);
