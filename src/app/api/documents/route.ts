...
export async function POST(request: NextRequest) {
  ...
  if (diagnosticOrderId && !(await prisma.diagnosticOrder.findFirst({ where: { id: diagnosticOrderId, patientId, organizationId } }))) return NextResponse.json({ error: "Diagnostic order not found." }, { status: 404 });
  ...
  const document = await prisma.storedDocument.create({ data: { organizationId, patientId, visitId, diagnosticOrderId, originalName: path.basename(file.name).slice(0, 255), contentType: file.type, sizeBytes: file.size, storageKey, checksumSha256, uploadedById: access.user.id } });
  ...
  return NextResponse.json({ id: document.id, originalName: document.originalName, contentType: document.contentType, sizeBytes: document.sizeBytes, createdAt: document.createdAt }, { status: 201 });
}
