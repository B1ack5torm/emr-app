...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const visit = await prisma.visit.findFirst({
    ...
    include: {
      patient: { include: { allergies: true, medicationStatements: { where: { status: "ACTIVE" } }, clinicalFlags: { where: { active: true } } } }, prescriptions: true, testsOrdered: true, imagingRecommendations: true,
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const imagingRecommendations = (Array.isArray(body.imagingRecommendations) ? body.imagingRecommendations : [])
  ...
  const visit = await prisma.$transaction(async (tx) => {
    ...
    await tx.imagingRecommendation.deleteMany({ where: { visitId: params.id } });
    ...
    return tx.visit.update({
      ...
      data: {
        ...
        imagingRecommendations: { create: imagingRecommendations.map((item: any) => ({
          code: optionalText(item.code, 100), name: String(item.name).trim().slice(0, 500),
          modality: String(item.modality || "Other").trim().slice(0, 100) || "Other",
          bodyPart: optionalText(item.bodyPart, 200),
          description: String(item.description || item.name).trim().slice(0, 1000),
        })) },
      ...
      include: { prescriptions: true, testsOrdered: true, imagingRecommendations: true, diagnoses: true, vitals: { orderBy: { measuredAt: "desc" } }, doctor: { select: { name: true } } },
    });
  });
