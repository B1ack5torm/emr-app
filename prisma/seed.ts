...
async function main() {
  ...
  for (const dayOfWeek of [1, 2, 3, 4, 5, 6]) await prisma.practitionerSchedule.upsert({ where: { practitionerId_dayOfWeek: { practitionerId: profile.id, dayOfWeek } }, update: {}, create: { practitionerId: profile.id, dayOfWeek, startMinute: 540, endMinute: 1020, appointmentMinutes: 30, breaks: { create: [{ startMinute: 780, endMinute: 840, label: "Lunch" }] } } });
  ...
  for (const service of [{ code: "CONSULT", name: "General consultation", category: "CONSULTATION" as const, unitPrice: 50000 }, { code: "CBC", name: "Complete blood count", category: "TEST" as const, unitPrice: 35000 }, { code: "XR-CHEST-2V", name: "Chest X-ray, 2 views", category: "IMAGING" as const, unitPrice: 60000 }]) {
